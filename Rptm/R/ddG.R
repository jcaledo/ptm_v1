...
#' @param protein eihter the 4-letter identifier of a PDB structure, or the amino acid sequence (one letter amino acid code) of a protein.
...
imutant <- function(protein, ch = "_", pos, newres = "",
  ...
  resp <- httr::POST(cgi_url, body = query_parameters)

  if (httr::http_error(resp)){
    stop(paste("HTTP communication failure: ", resp$status_code, sep = ""))
  } else {
...
foldx.mut <- function(pdb, ch, pos, newres = "", pH = 7,
  ...
  # Regardeless the method beloging to FoldX (either BuilModel or PositionScan), we will
  ...
  if (repaired){
    # file.remove(paste(id, "_Repair.fxout", sep = ""))
  ...
  if (method == 'buildmodel'){ ## ------------------------------ Method FoldX-BuildModel
    ...
    if (newres %in% aa){
      ...
      stop("Something went wrong when parsing the result file")
    }
    ...
    system(AB)
    ## -------------- Parsing the result page -------------- ##
    con <- file(paste('PS_', id, "_Repair_scanning_output.txt", sep = ""), 'r')
    lines <- readLines(con)
    close(con)
    ## --------------- Output dataframe -------------------- ##
    if (newres %in% c(aa, nst_aa)){
...
#' @param prot eihter the 4-letter identifier of a PDB structure, or the amino acid sequence (one letter amino acid code) of a protein.
#' @param ch a letter identifying the chain of interest.
#' @param pos the position, in the primary structure, of the residue to be mutated.
#' @param pH a numeric value between 0 and 14.
#' @param Te a numeric value indicating the temperature in degrees Celsius.
...
ddG.profile <- function(prot, ch, pos, pH = 7, Te = 25){
  ...
  t <- imutant(protein = prot, ch = ch, pos = pos, pH = pH, Te = Te)
  ddG <- t$DDG
  names(ddG) <- t$NEW
...
#' @param pH a numeric value between 0 and 14.
#' @details The current fucntion uses FoldX to build the model of the modified protein. Currently, FoldX does not allow to change Met by MetO, so we
