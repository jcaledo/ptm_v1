...
Let's focus on the function **search.go()**. This function, returns a dataframe  with the GO terms associated to the query of interest. For instance, if we pass the argument 'oxidative stress':

```{r, echo = TRUE, include = FALSE}
o <- search.go('oxidative stress')
```

We obtain the GO terms linked to this query. For instance, the first ten rows in our example would look like:

```{r}
kable(o[1:10, c(1,3,5)])
```
