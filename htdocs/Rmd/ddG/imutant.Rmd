...
### Description
Computes changes in the stability of a protein after a residue mutation (ΔΔG), using a machine-learning approach.

### Usage
imutant(protein, ch = "_", pos, newres = "", timeout = 60)

### Arguments
_protein_	eihter the 4-letter identifier of a PDB structure, or the amino acid sequence (one letter amino acid code) of a protein.

_ch_ a letter identifying the chain of interest.

_pos_ the position, in the primary structure, of the residue to be mutated.

_newres_ the one letter code of the residue to be incorporated. When a value is not entered for this parameter, then the function will compute DDG for the mutation to any possible amino acid.

_timeout_ maximum time to wait, in seconds, for a response from the I-Mutant server.
...
### References
Capriotti et al (2005) [Nucl. Ac. Res. 33:W306-W310](https://www.ncbi.nlm.nih.gov/pubmed/15980478).
...
When interpreting results arising from the use of *foldx.mut()* a practical rule of thumb may be as follows:

![](./thumb.png)


To illustrate the use of the functions *imutant()* and *foldx.mut()*, we will use as a model the β1 domain of streptococcal protein G (Gβ1), which has been extensively [characterized experimentally](https://www.pnas.org/content/116/33/16367) in terms of thermodynamic stability. 
...
Nevertheless, if the computing time is not a concern for you, then you may wish to use *foldx.mut()* instead of *imutant()*. The function *foldx.mut()* will repair your structure before carrying out the computation. To do that, FoldX identify those residues which have bad torsion angles, or VanderWaals' clashes, or total energy, and repairs them. The function *foldx.mut()* will take care of this repair process, so you only have to indicate which of the two alternative methods that *foldx.mut()* offers you whant to use to assess the requested change in stability: ['buildmodel'](http://foldxsuite.crg.eu/command/BuildModel) 
...
Warning: the function *foldx.mut()* makes an extensive use of commands invoking the OS, reason why it may not work properly on Windows. If that is your case, you always can use *imutant()*.
